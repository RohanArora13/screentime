...
# previous directory
#screentime_dir= os.path.normpath(os.getcwd() + os.sep + os.pardir)

screentime_dir= os.getcwd()
...
def runSoftware():
    ...
    #print('"'+str(pathlib.Path().resolve())+'\\'+constants.updater_name+'.py"')
       # os.system('"'+str(screentime_dir)+'\\'+constants.software_name+'.exe"')
       os.startfile('"'+str(screentime_dir)+'\\'+constants.software_name+'.exe"')
    except:
...
def addToReg():
    ...
    try:
        ...
        print ("added exe in regex successful")
...
def downloadFile(url):
    ...
    #TODO remove test
    print("extracting update.. please wait")
    #z.extractall(str(pathlib.Path().resolve()))

    #pz = open(io.BytesIO(r.content), 'rb')
    ...
    for name in z.namelist():
        ...
        #     pass
        #print(str(os.getcwd())+str("name = "+str(name)))
        z.extract(name, str(screentime_dir))
...
def crossCheck():
    ...
    local_version=constants.software_version
    # get version via pickle


    try:
        file = open(str(screentime_dir)+"\\"+str(constants.software_version_pickle_name), 'rb')
        local_version = pickle.load(file)
        file.close()
        print("current version "+str(local_version))
    except:
    ...
    if(int(online_version)>int(local_version)):
        #admin.run_as_admin()
        print("update found..")
        ...
        #downlaod and extract new software
...
def addPickle():
    file = open(constants.updater_version_pickle_name, 'wb')
